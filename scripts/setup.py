...
Run with: python3 scripts/setup.py
"""
...
try:
    ...
    print(
        "Error: 'requests' is required for this interpreter. Install with:\n"
...
MODAL_DATA_VOLUME = "oura-health-data"
...
def exception_label(exc: BaseException) -> str:
    """Describe a failure without echoing token-bearing request URLs."""
...
def validate_anthropic_key(api_key: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        return False, f"Network error ({exception_label(exc)})"
...
def save_env_file(config: dict, filepath: Path = ENV_FILE):
    """Save configuration to a user-readable-only ``.env`` file."""
...
def create_modal_secrets(config: dict) -> bool:
    """Create Modal secrets without placing secret values in process arguments."""
    secrets_config = [
    ...
    for secret_name, values in secrets_config:
        ...
        try:
            with tempfile.NamedTemporaryFile(
                mode="w",
                prefix=f"oura-{secret_name}-",
                suffix=".env",
                delete=False,
            ) as dotenv_file:
                os.fchmod(dotenv_file.fileno(), 0o600)
                for key, value in values.items():
                    dotenv_file.write(f"{key}={value}\n")
                dotenv_path = Path(dotenv_file.name)
...
def upload_profile_to_modal(profile: dict) -> Tuple[bool, str]:
    """Upload profile JSON without exposing health context in process arguments."""
    profile_path = None
    try:
        with tempfile.NamedTemporaryFile(
            mode="w",
            ...
            os.fchmod(profile_file.fileno(), 0o600)
        ...
        if result.returncode == 0:
            return True, ""
        return False, (result.stderr or "Modal volume upload failed").strip()[:100]
    except subprocess.TimeoutExpired:
        return False, "Profile upload timed out"
    except Exception as exc:
        return False, f"Profile upload failed ({exception_label(exc)})"
    finally:
...
def register_telegram_webhook(
    ...
    try:
        ...
        print_error(f"Network error ({exception_label(exc)})")
        return False
...
def apply_profile_timezone(profile: Optional[dict], timezone_name: str) -> Optional[dict]:
    """Apply the final schedule timezone to a profile built earlier in setup."""
    if profile is None:
        return None
    profile.setdefault("user", {})["timezone"] = timezone_name
    return profile
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python3 scripts/setup.py              # Full interactive setup
  python3 scripts/setup.py --update     # Update existing credentials
  python3 scripts/setup.py --local-only # Only create .env file
  python3 scripts/setup.py --skip-deploy # Skip Modal deployment
        """,
    )
    parser.add_argument(
        "--update",
    ...
    print("Oura deprecated personal access tokens in December 2025.")
    print("This version can keep using an existing OURA_ACCESS_TOKEN, but does")
    print("not yet implement Oura's recommended OAuth2 flow for new access.\n")
    print_dim("   Keep a working existing token until the OAuth migration is complete.\n")
    ...
    if not chat_id:
        ...
        if choice == "1":
            ...
            print("  2. Call Telegram's getUpdates API with your bot token")
    ...
    if not args.local_only:
        ...
        if not modal_installed:
            ...
            if confirm("Install Modal CLI now?"):
                ...
                print_info("Skipping Modal. Run 'python3 -m pip install modal' later to deploy.")
        ...
        if modal_installed and modal_authenticated and not args.skip_deploy:
            ...
            profile = apply_profile_timezone(profile, tz_name)
        ...
        if modal_installed and modal_authenticated:
            ...
            if success:
                ...
                if profile:
                    ...
                    profile_uploaded, profile_error = upload_profile_to_modal(profile)
                    if profile_uploaded:
                        print_success("Profile saved to Modal volume")
                    else:
                        print_error("Failed to upload profile")
                        if profile_error:
                            print_dim(f"   {profile_error}")
        else:
    ...
    if args.local_only:
        ...
        print("To deploy to Modal later, run: python3 scripts/setup.py --update")
        print()
        return
    ...
    if not deployed:
        ...
        print("  python3 scripts/setup.py --update")
